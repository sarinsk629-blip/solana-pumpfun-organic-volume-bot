...
const randSOLAmount = () => randInt(BUY_AMOUNT_MIN, BUY_AMOUNT_MAX) //0.005SOL ~ 0.01SOL
...
const randTokenAmount = async (BASEMINT: PublicKey, wallet: PublicKey) => {
    ...
    console.log("🚀 ~ randTokenAmount ~ amountt:", amount)
    return BigInt(randInt(Math.floor(Number(amount.value.amount) * 0.1), Math.floor(Number(amount.value.amount) * 0.6)));
}